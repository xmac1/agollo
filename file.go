...
func loadConfigFile(configDir string) (*ApolloConfig,error){
	...
	logger.Println("load config file from :",configFilePath)
	file, e := os.Open(configFilePath)
	if e!=nil{
		logger.Println("loadConfigFile fail,error:",e)
		return nil,e
	}
	defer file.Close()
	config:=&ApolloConfig{}
